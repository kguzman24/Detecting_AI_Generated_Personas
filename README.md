...
### Rationale
The InstaFake dataset contains 1,194 account records (994 real and 200 fake) while the Kaggle face dataset contains 9,630 images (5,000 real human faces, 4,680 AI-generated faces). To create a 1:1 pairing and maximize the use of the image data, 8,436 synthetic account records were generated by sampling from distributions fit to the real account data (mean/std for continuous features, empirical probability for binary features). The account records fake/real ratio (~16.7% fake) is preserved in synthetic generation. Fake accounts are paired
with AI-generated faces and real accounts with real human faces. Synthetic records are flagged with `is_synthetic=True`.
